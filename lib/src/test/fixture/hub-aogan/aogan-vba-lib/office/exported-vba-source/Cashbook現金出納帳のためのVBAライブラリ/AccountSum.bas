...
Public Sub 小計の表を作る(ByRef ws現金出納記録 As Worksheet)
    ...
    Dim item As Variant
    For Each item In unique勘定科目名の列
        Call 勘定科目の小計を算出する(ws現金出納記録, item)
    Next
...
Public Sub 現金出納記録ワークシートが無ければ作る()
    ...
    If Not KzVerifyWorksheetExists(TheSheetName) Then
        Debug.Print "現金出納記録ワークシートが未だ無いので作ります"
